...
int PrimoGradientMaterial::compare(const QSGMaterial* other) const
{
    ...
    if (colorTop.rgba() != o->colorTop.rgba())
        return colorTop.rgba() < o->colorTop.rgba() ? -1 : 1;
    if (colorBottom.rgba() != o->colorBottom.rgba())
        return colorBottom.rgba() < o->colorBottom.rgba() ? -1 : 1;
    if (!qFuzzyCompare(opacity, o->opacity))
        return opacity < o->opacity ? -1 : 1;
    return 0;
}
...
QSGGeometryNode* createGradientRectNode(const QRectF& rect, const QColor& top, const QColor& bottom, float opacity)
{
    ...
    v[0].g = top.green();
    v[0].b = top.blue();
    v[0].a = int(top.alpha() * opacity);
    v[1].x = rect.x() + rect.width();
    v[1].y = rect.y();
    v[1].r = top.red();
    v[1].g = top.green();
    v[1].b = top.blue();
    v[1].a = int(top.alpha() * opacity);
    v[2].x = rect.x();
    v[2].y = rect.y() + rect.height();
    v[2].r = bottom.red();
    v[2].g = bottom.green();
    v[2].b = bottom.blue();
    v[2].a = int(bottom.alpha() * opacity);
    v[3].x = rect.x() + rect.width();
    v[3].y = rect.y() + rect.height();
    v[3].r = bottom.red();
    v[3].g = bottom.green();
    v[3].b = bottom.blue();
    v[3].a = int(bottom.alpha() * opacity);
