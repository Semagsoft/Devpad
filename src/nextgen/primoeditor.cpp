...
PrimoEditor::PrimoEditor(QQuickItem* parent) : QQuickItem(parent)
{
    ...
    connect(m_doc, &PrimoDocument::textChanged, this, [this]() {
        m_linesDirty = true;
    ...
    connect(m_doc, &PrimoDocument::languageChanged, this, [this]() {
    ...
    connect(m_highlightController, &PrimoHighlighterController::highlighted, this, [this](int version, int first, QVector<QVector<QTextLayout::FormatRange>> fmts){
        applyHighlightResult(version, first, fmts);
    });

    connect(this, &PrimoEditor::textChanged, this, [this]() {
    ...
    connect(this, &PrimoEditor::fontChanged, this, [this]() {
        ensureMetrics();
...
void PrimoEditor::setFilePath(const QString& path) { if (m_doc) m_doc->setFilePath(path); }
QString PrimoEditor::text() const { return m_doc ? m_doc->text() : QString(); }
void PrimoEditor::setText(const QString& t) { if (m_doc) m_doc->setText(t); }
QString PrimoEditor::language() const { return m_doc ? m_doc->language() : QString(); }
void PrimoEditor::setLanguage(const QString& lang) { if (m_doc) m_doc->setLanguage(lang); }
QString PrimoEditor::encoding() const { return m_encoding; }
void PrimoEditor::setEncoding(const QString& enc) { if (m_encoding != enc) { m_encoding = enc; emit encodingChanged(); } }
...
void PrimoEditor::setTabWidth(int w) { if (m_tabWidth != w) { m_tabWidth = w; emit tabWidthChanged(); update(); } }
bool PrimoEditor::wordWrap() const { return m_wordWrap; }
void PrimoEditor::setWordWrap(bool w) { if (m_wordWrap != w) { m_wordWrap = w; emit wordWrapChanged(); update(); } }
int PrimoEditor::cursorLine() const { return m_cursorLine; }
void PrimoEditor::setCursorLine(int line) { if (m_cursorLine != line) { m_cursorLine = line; emit cursorChanged(); update(); } }
int PrimoEditor::cursorColumn() const { return m_cursorColumn; }
void PrimoEditor::setCursorColumn(int col) { if (m_cursorColumn != col) { m_cursorColumn = col; emit cursorChanged(); update(); } }
...
qreal PrimoEditor::lineHeight() const { return m_lineHeight; }
QColor PrimoEditor::backgroundColor() const { return m_bg; }
void PrimoEditor::setBackgroundColor(const QColor& c) { if (m_bg != c) { m_bg = c; emit backgroundColorChanged(); update(); } }
QColor PrimoEditor::foregroundColor() const { return m_fg; }
void PrimoEditor::setForegroundColor(const QColor& c) { if (m_fg != c) { m_fg = c; emit foregroundColorChanged(); update(); } }
bool PrimoEditor::gutterVisible() const { return m_gutterVisible; }
void PrimoEditor::setGutterVisible(bool v) { if (m_gutterVisible!=v){m_gutterVisible=v; emit gutterVisibleChanged(); ensureMetrics(); update();}}
bool PrimoEditor::relativeNumbers() const { return m_relativeNumbers; }
void PrimoEditor::setRelativeNumbers(bool v){ if(m_relativeNumbers!=v){m_relativeNumbers=v; emit relativeNumbersChanged(); update();}}
bool PrimoEditor::isReadOnly() const { return m_readOnly; }
void PrimoEditor::setReadOnly(bool ro){ if(m_readOnly!=ro){m_readOnly=ro; emit readOnlyChanged(); update();}}
bool PrimoEditor::minimapVisible() const { return m_minimapVisible;}
void PrimoEditor::setMinimapVisible(bool v){ if(m_minimapVisible!=v){m_minimapVisible=v; emit minimapVisibleChanged(); update();}}
qreal PrimoEditor::gutterWidth() const { return m_gutterWidth; }
PrimoDocument* PrimoEditor::document() const { return m_doc; }

void PrimoEditor::toggleBookmark(int line){ if(line<0) return; if(m_bookmarks.contains(line)) m_bookmarks.remove(line); else m_bookmarks.insert(line); emit bookmarksChanged(); update();}
void PrimoEditor::toggleBookmarkCurrent(){ toggleBookmark(m_cursorLine); }
bool PrimoEditor::hasBookmark(int line) const { return m_bookmarks.contains(line); }
QList<int> PrimoEditor::bookmarkLines() const { QList<int> l = m_bookmarks.values(); std::sort(l.begin(), l.end()); return l; }
...
void PrimoEditor::loadFile(const QString& path)
{
    ...
    if (ok) {
        ...
        m_bg = m_theme.background;
        m_fg = m_theme.foreground;
        m_cursorLine = 0; m_cursorColumn = 0;
        emit cursorChanged();
...
{
    if (m_readOnly) { emit fileSaved(false, QStringLiteral("Read-only (large file >50MB)")); return false; }
    if (!m_doc || m_doc->filePath().isEmpty()) return false;
    QString error; bool ok = m_doc->saveToFile(m_doc->filePath(), &error);
    if (ok) m_doc->setModified(false);
    emit fileSaved(ok, error);
    return ok;
}
bool PrimoEditor::saveAs(const QString& path)
{
    if (m_readOnly) { emit fileSaved(false, QStringLiteral("Read-only")); return false; }
    if (!m_doc) return false;
    QString error; bool ok = m_doc->saveToFile(path, &error);
    if (ok) { m_doc->setFilePath(path); m_doc->setModified(false); }
    emit fileSaved(ok, error);
    m_linesDirty = true;
    update();
    return ok;
...
{
    bool changed = false;
    if (m_cursorLine != line) { m_cursorLine = line; changed = true; }
    if (m_cursorColumn != column) { m_cursorColumn = column; changed = true; }
    if (changed) { emit cursorChanged(); update(); }
}
void PrimoEditor::insertAtCursor(const QString& s)
{
    ...
    if (m_cursorLine >= lines.size()) { while (lines.size() <= m_cursorLine) lines.append(QString()); }
    QString& line = lines[m_cursorLine];
    if (m_cursorColumn < 0) m_cursorColumn = 0;
    if (m_cursorColumn > line.size()) m_cursorColumn = line.size();
...
void PrimoEditor::backspaceAtCursor()
{
    ...
    QString txt = m_doc->text();
    QStringList lines = txt.split(QLatin1Char('\n'));
    if (m_cursorLine < 0 || m_cursorLine >= lines.size()) return;
    QString& line = lines[m_cursorLine];
    if (m_cursorColumn > 0) {
        ...
        // bookmarks shift
        QSet<int> nb;
        for(int b: m_bookmarks){ if(b==m_cursorLine+1) nb.insert(m_cursorLine); else if(b>m_cursorLine+1) nb.insert(b-1); else nb.insert(b); }
        m_bookmarks = nb;
    } else return;
    m_doc->setText(lines.join(QLatin1Char('\n')));
    emit cursorChanged();
...
void PrimoEditor::keyPressEvent(QKeyEvent* event)
{
    ...
    if (event->modifiers() & Qt::ControlModifier && event->key()==Qt::Key_B) { toggleBookmarkCurrent(); event->accept(); return; }
    if (event->key() == Qt::Key_F2) { // next bookmark? simple toggle for now
        toggleBookmarkCurrent(); event->accept(); return;
    }
    if (event->key() == Qt::Key_Backspace) { backspaceAtCursor(); event->accept(); return; }
    if (event->key() == Qt::Key_Return || event->key() == Qt::Key_Enter) { insertAtCursor(QStringLiteral("\n")); m_cursorLine++; m_cursorColumn = 0; emit cursorChanged(); event->accept(); return; }
    if (event->key() == Qt::Key_Left) { if (m_cursorColumn > 0) m_cursorColumn--; else if (m_cursorLine > 0) { m_cursorLine--; QStringList ls = m_doc->text().split(QLatin1Char('\n')); if (m_cursorLine < ls.size()) m_cursorColumn = ls[m_cursorLine].size(); } emit cursorChanged(); update(); event->accept(); return; }
    if (event->key() == Qt::Key_Right) { QStringList ls = m_doc->text().split(QLatin1Char('\n')); if (m_cursorLine < ls.size() && m_cursorColumn < ls[m_cursorLine].size()) m_cursorColumn++; else if (m_cursorLine + 1 < ls.size()) { m_cursorLine++; m_cursorColumn = 0; } emit cursorChanged(); update(); event->accept(); return; }
    if (event->key() == Qt::Key_Up) { if (m_cursorLine > 0) m_cursorLine--; emit cursorChanged(); update(); event->accept(); return; }
    if (event->key() == Qt::Key_Down) { QStringList ls = m_doc->text().split(QLatin1Char('\n')); if (m_cursorLine + 1 < ls.size()) m_cursorLine++; emit cursorChanged(); update(); event->accept(); return; }
...
void PrimoEditor::mousePressEvent(QMouseEvent* event)
{
    ...
    if (event->position().x() < gutterW) {
        ...
        if (line >=0) toggleBookmark(line);
        event->accept(); return;
    }
    int line = int((event->position().y() - 4) / m_lineHeight);
    if (line < 0) line = 0;
    QStringList ls = m_doc ? m_doc->text().split(QLatin1Char('\n')) : QStringList{QString()};
    if (line >= ls.size()) line = qMax(0, ls.size() - 1);
    int col = 0;
    if (line < ls.size()) {
...
void PrimoEditor::focusInEvent(QFocusEvent* e) { QQuickItem::focusInEvent(e); update(); }
void PrimoEditor::focusOutEvent(QFocusEvent* e) { QQuickItem::focusOutEvent(e); update(); }

QString PrimoEditor::expandedTab(const QString& line) const
{
    if (!line.contains(QLatin1Char('\t'))) return line;
    QString out;
    out.reserve(line.size() + 8);
    for (QChar ch : line) {
        if (ch == QLatin1Char('\t')) out += QString(m_tabWidth, QLatin1Char(' '));
        else out += ch;
    }
    return out;
}
...
{
    if (!m_doc || m_doc->text().isEmpty()) return;
    // Determine language already set
    QString lang = m_doc->language();
    if (lang.isEmpty()) lang = m_doc->language(); // fallback
    ...
    if (firstLine >=0 && lastLine>=0) { first = firstLine; last = lastLine; }
    else if (isLarge) {
        // visible range
        ...
        if (flick) {
            ...
            if (v.isValid()) contentY = v.toReal();
            else if (flick->parentItem()) { QVariant v2 = flick->parentItem()->property("contentY"); if(v2.isValid()) contentY=v2.toReal(); }
            if (h <=0) h = 800;
        }
    ...
    if (!isLarge && (last - first) < 500) {
        ...
        for (int i=first;i<=last;++i) chunk.append(m_doc->lineAt(i));
        QVector<QVector<QTextLayout::FormatRange>> res;
        res.reserve(chunk.size());
        for (auto &ln: chunk) res.append(PrimoHighlighter::formatsForLine(ln, lang, m_theme));
...
{
    if (version != m_highlightVersion) return;
    if (firstLine <0 || firstLine >= m_highlightCache.size()) return;
...
QSGNode* PrimoEditor::updatePaintNode(QSGNode* oldNode, UpdatePaintNodeData* data)
{
    ...
    while (root->firstChild()) delete root->firstChild();
    ...
    if (m_linesDirty || m_linesCache.isEmpty()) {
        QString txt = m_doc ? m_doc->text() : QString();
        if (txt.isEmpty()) lines = QStringList{QString()};
        else lines = txt.split(QLatin1Char('\n'));
        m_linesCache = lines;
        m_linesDirty = false;
    ...
    if (flick) {
        ...
        if (v.isValid()) contentY = v.toReal();
            ...
            if (v2.isValid()) contentY = v2.toReal();
        }
    }
    if (contentY > 0) {
    ...
    if (m_highlightDirty) {
        ...
        bool need=false;
        for(int i=firstLine;i<=lastLine && i < m_highlightCache.size(); ++i) if(m_highlightCache[i].isEmpty() && !lines[i].isEmpty()) {need=true; break;}
        if(need) scheduleHighlight(firstLine, lastLine);
    }
    ...
    if (m_gutterVisible) {
        for (int i = firstLine; i <= lastLine; ++i) {
            ...
            if (m_relativeNumbers && hasActiveFocus()) {
                ...
                if (i==m_cursorLine) numStr = QString::number(i+1); else numStr = QString::number(rel);
            } else {
    ...
    if (hasActiveFocus() && m_cursorLine >= firstLine && m_cursorLine <= lastLine) {
        ...
        QSGGeometryNode* grad = createGradientRectNode(QRectF(cx, cy, 2, m_lineHeight - 2), QColor(0xc4,0xa7,0xe7,200), QColor(0x89,0xb4,0xfa,200), 0.95f);
