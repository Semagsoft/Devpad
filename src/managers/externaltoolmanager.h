...
public:
    ...
    [[nodiscard]] QList<ExternalTool> tools() const;
    void setTools(const QList<ExternalTool>& tools);

    [[nodiscard]] QString resolveVariables(const QString& template_, const QString& filePath, const QString& projectDir, const QString& selectedText,
                                           int lineNumber) const;
    [[nodiscard]] QString workingDirForTool(const ExternalTool& tool, const QString& filePath, const QString& projectDir) const;

    // Parse arguments string into individual tokens, respecting shell quoting
    [[nodiscard]] static QStringList parseArguments(const QString& args);

    // Escape a string for safe use in a POSIX shell (single-quote wrapping)
    [[nodiscard]] static QString shellEscape(const QString& s);
    ...
    bool runTool(int index, const QString& filePath, const QString& projectDir, const QString& selectedText, int lineNumber,
                 const std::function<void(const QString&)>& terminalSender, QWidget* parent = nullptr);
