...
QString PrintManager::generateHighlightedHtml(CodeEditor *editor) const {
    ...
    while (pos < length) {
        ...
        if (!fgColor.isValid()) fgColor = defaultColor;
        QColor bgColor = lexer->paper(style);
        if (!bgColor.isValid()) bgColor = defaultPaper;
        ...
        QString styleAttr = QString("color: #%1; background-color: #%2;")
            .arg(fgColor.name().mid(1))
            .arg(bgColor.name().mid(1));
