...
RecentFilesHelper::RecentFilesHelper(QObject* parent) : QObject(parent)
{
}
...
QMenu* RecentFilesHelper::createMenu(QWidget* parentWidget)
{
    ...
    for (int i = 0; i < 10; ++i)
    {
        ...
        connect(m_fileActions[i], &QAction::triggered, this,
                [this, i]()
                {
                    if (m_fileActions[i])
                    {
    ...
    connect(m_clearAction, &QAction::triggered, this, [this]() { emit clearRecentFilesTriggered(); });
