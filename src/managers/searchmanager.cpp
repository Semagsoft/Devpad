...
SearchManager::SearchManager(QMainWindow* parent, TabManager* tabManager) : QObject(parent), m_parent(parent), m_tabManager(tabManager)
{
}
...
void SearchManager::findNext()
{
    ...
    {
        performSearch(m_lastSearchText, m_lastUseRegex, m_lastMatchCase, m_lastMatchWholeWord, true);
    }
    else if (container)
    {
        container->showFindBar();
...
void SearchManager::findPrevious()
{
    ...
    if (!m_lastSearchText.isEmpty())
    {
        performSearch(m_lastSearchText, m_lastUseRegex, m_lastMatchCase, m_lastMatchWholeWord, false);
    }
    else if (container)
    {
        container->showFindBar();
...
void SearchManager::performSearch(const QString& text, bool regex, bool caseSensitive, bool wholeWord, bool searchForward)
{
    auto* editor = currentEditor();
    if (!editor || text.isEmpty())
        return;

    bool found = editor->findFirst(text, regex, caseSensitive, wholeWord, true, searchForward);
